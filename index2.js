...
const groupUrls = [
  process.env.FACEBOOK_GROUP11,
  process.env.FACEBOOK_GROUP12,
  process.env.FACEBOOK_GROUP13,
  process.env.FACEBOOK_GROUP14,
  process.env.FACEBOOK_GROUP15,
  process.env.FACEBOOK_GROUP16,
  process.env.FACEBOOK_GROUP17,
  process.env.FACEBOOK_GROUP18,
  process.env.FACEBOOK_GROUP19,
  process.env.FACEBOOK_GROUP20,
];
...
const SEEN_POSTS_FILE = "seen-posts-2.json";
...
async function reloadAllGroups(pages) {
  for (let start = 0; start < pages.length; start += BATCH_SIZE) {
    ...
    const firstGroup = start + 11;
    const lastGroup = Math.min(start + BATCH_SIZE + 10, 20);

    console.log(`\nRefreshing Groups ${firstGroup} → ${lastGroup}...`);

    await Promise.all(
      batch.map((page, index) => reloadPage(page, start + index + 11)),
    );
...
async function getPostUrls(pages) {
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    console.log(`Group ${i + 11}: Found ${urls.length} post links`);
...
async function startMonitoring() {
  ...
  const browser = await puppeteer.launch({
    ...
    userDataDir: process.env.USER_DATA_DIR_2,
  });
  ...
  for (let i = 0; i < pages.length; i++) {
    await openGroup(pages[i], groupUrls[i], i + 11);
  }
