...
const groupUrls = [
  process.env.FACEBOOK_GROUP21,
  process.env.FACEBOOK_GROUP22,
  process.env.FACEBOOK_GROUP23,
  process.env.FACEBOOK_GROUP24,
  process.env.FACEBOOK_GROUP25,
  process.env.FACEBOOK_GROUP26,
  process.env.FACEBOOK_GROUP27,
  process.env.FACEBOOK_GROUP28,
  process.env.FACEBOOK_GROUP29,
  process.env.FACEBOOK_GROUP30,
];
...
const SEEN_POSTS_FILE = "seen-posts-3.json";
...
async function reloadAllGroups(pages) {
  for (let start = 0; start < pages.length; start += BATCH_SIZE) {
    ...
    const firstGroup = start + 21;
    const lastGroup = Math.min(start + BATCH_SIZE + 20, 30);

    console.log(`\nRefreshing Groups ${firstGroup} → ${lastGroup}...`);

    await Promise.all(
      batch.map((page, index) => reloadPage(page, start + index + 21)),
    );
...
async function getPostUrls(pages) {
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    console.log(`Group ${i + 21}: Found ${urls.length} post links`);
...
async function startMonitoring() {
  ...
  const browser = await puppeteer.launch({
    ...
    userDataDir: process.env.USER_DATA_DIR_3,
  });
  ...
  for (let i = 0; i < pages.length; i++) {
    await openGroup(pages[i], groupUrls[i], i + 21);
  }
